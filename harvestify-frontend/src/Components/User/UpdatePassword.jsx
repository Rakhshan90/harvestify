...
const UpdatePassword = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
              dispatch(userUpdatePasswordAction(values));
        },
        validationSchema: formSchema,
    });

    const updatePasswordData = useSelector(store => store?.users);
    const {loading, appErr, serverErr, isPasswordUpdated} = updatePasswordData;
    if(isPasswordUpdated) navigate('/'); 
            ...
            <div className="w-full  p-4">
                <div className="flex flex-col justify-center max-w-md mx-auto h-full py-12">
                    <form onSubmit={formik.handleSubmit}>
                        <h1 className="text-3xl font-bold font-heading mb-4">Update Your Password</h1>
                        {/* display error */}
                        {appErr || serverErr ? (
